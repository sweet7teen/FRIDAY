...
start_date = int(input("Enter the start date (YYMM): "))
end_date = int(input("Enter the end date (YYMM): "))
...
for num in range(start_date, end_date + 1):
    period = str(num)
    table_names = "mstran_" + period
    print(table_names)

    query_bpb = (
        "SELECT SHOP,DOCNO DOCNO,DATE(TGL1) TGL1,CONCAT(YEAR(TGL1),'-',SHOP,'-',DOCNO,IF(RTYPE='B','',IF(RTYPE='K','-CM',CONCAT('-',RTYPE))),IF(LEFT(TOKO,1)='G', CONCAT('-',SUB_BKP),'')) AS INV_EDP,TOKO,ISTYPE,SUB_BKP,QTY,SUM(GROSS) GROSS,SUM(PPN) PPN,SUM(DISC_05) DISC_05 FROM %s WHERE RTYPE='B' AND ISTYPE NOT IN ('V','RMB','GGC') GROUP BY SHOP,DOCNO,RTYPE,SUB_BKP,ISTYPE;"
        % (table_names)
    )

    query_nrb = (
        "SELECT SHOP,DOCNO DOCNO,DATE(TGL1) TGL1,CONCAT(YEAR(TGL1),'-',SHOP,'-',DOCNO,IF(RTYPE='B','',IF(RTYPE='K','-CM',CONCAT('-',RTYPE))),IF(LEFT(TOKO,1)='G', CONCAT('-',SUB_BKP),'')) AS INV_EDP,TOKO,ISTYPE,SUB_BKP,QTY,SUM(GROSS) GROSS,SUM(PPN) PPN,SUM(DISC_05) DISC_05 FROM %s WHERE RTYPE='K' GROUP BY SHOP,DOCNO,RTYPE,SUB_BKP,ISTYPE;"
        % (table_names)
    )

    query_vir = (
        "SELECT SHOP,DOCNO DOCNO,DATE(TGL1) TGL1,CONCAT(YEAR(TGL1),'-',SHOP,'-',DOCNO,IF(RTYPE='B','',IF(RTYPE='K','-CM',CONCAT('-',RTYPE))),IF(LEFT(TOKO,1)='G', CONCAT('-',SUB_BKP),'')) AS INV_EDP,TOKO,ISTYPE,SUB_BKP,QTY,SUM(GROSS) GROSS,SUM(PPN) PPN,SUM(DISC_05) DISC_05 FROM %s WHERE RTYPE='B' AND SHOP LIKE 'F%%' AND ISTYPE='RMB' GROUP BY SHOP,`DATE`"
        % (table_names)
    )

    query_babkl = (
        "SELECT SHOP,DOCNO DOCNO,DATE(TGL1) TGL1,CONCAT(YEAR(TGL1),'-',SHOP,'-',DOCNO,IF(RTYPE='B','',IF(RTYPE='K','-CM',CONCAT('-',RTYPE))),IF(LEFT(TOKO,1)='G', CONCAT('-',SUB_BKP),'')) AS INV_EDP,TOKO,ISTYPE,SUB_BKP,QTY,SUM(GROSS) GROSS,SUM(PPN) PPN,SUM(DISC_05) DISC_05 FROM %s WHERE ISTYPE='BABKL' GROUP BY SHOP,DOCNO,RTYPE,SUB_BKP,ISTYPE;"
        % (table_names)
    )
