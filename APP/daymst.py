...
for num in range(start_date, end_date + 1):
    ...
    query_bpb = (
        "SELECT SHOP,BUKTI_NO DOCNO,DATE(TGL1) TGL1,CONCAT(YEAR(TGL1),'-',SHOP,'-',BUKTI_NO,IF(RTYPE='B','',IF(RTYPE='K','-CM',CONCAT('-',RTYPE))),IF(LEFT(TOKO,1)='G', CONCAT('-',SUB_BKP),'')) AS INV_EDP,TOKO,ISTYPE,SUB_BKP,QTY,SUM(GROSS) GROSS,SUM(PPN) PPN,SUM(DISC05) DISC05 FROM %s WHERE RTYPE='B' AND ISTYPE<>'V' GROUP BY SHOP,BUKTI_NO,RTYPE,SUB_BKP,ISTYPE;"
        % (table_names)
    )
