...
# source_folder_path = os.path.join(os.path.dirname(__file__), 'source')
    ...
    if os.path.isfile(os.path.join(hr_result, file_name)):
        ...
        try:
            ...
            if df.exists():
                ...
                df = pd.read_csv(zf.open(wt, 'r'), names=["RECID","KDTOKO","TGLTRX","DOCNO","DIV","KDSUPPLIER","INVNO","PRDCD","QTY","PRICE","GROSS","PPN","PRICE_IDM","PPNBM_IDM","PPNRP_IDM","KETERANGAN","DPP_JUAL_KONS","PPN_JUAL_KONS","BKP","SUB_BKP","RATE_PPN","KATEGORI"], sep='|', header=None)
                cek = len(df)
                if cek > 0:
                    df.to_csv(path_result + wt, index=False,
                              sep='|', header=None)
